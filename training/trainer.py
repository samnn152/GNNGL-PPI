...
from common.utils import Metrictor_PPI, print_file
from training.ui import TrainingObserver
...
def _log_line(message, save_file_path=None, observer=None):
    if observer is not None and observer.suppress_step_logs:
        ...
        return
    print_file(message, save_file_path=save_file_path)
...
def _batch_metrics(output, label, device):
    prediction = _predict_from_logits(output, device)
...
def _train_epoch(model, graph, loss_asl, optimizer, device, batch_size, epoch, got, observer):
    model.train()
    ...
    for step in range(steps):
        batch_number = step + 1
        edge_ids = _edge_batch(graph.train_mask_got if got else graph.train_mask, step, batch_size)
        observer.on_phase(
            "Training batch",
            "{}/{}: forward pass on {} edges".format(batch_number, steps, len(edge_ids)),
        )
        output, label = _forward_train_batch(model, graph, edge_ids, device, got)
        observer.on_phase(
            "Training batch",
        ...
        metrics, _ = _batch_metrics(output, label, device)
...
def _validate_epoch(model, graph, loss_asl, device, batch_size, observer):
    model.eval()
    ...
    with torch.no_grad():
        for step in range(valid_steps):
            batch_number = step + 1
            edge_ids = _edge_batch(graph.val_mask, step, batch_size)
            observer.on_phase(
                "Validation batch",
                "{}/{}: forward pass on {} edges".format(batch_number, valid_steps, len(edge_ids)),
            )
            output, label = _forward_valid_batch(model, graph, edge_ids, device)
            observer.on_phase(
                "Validation batch",
            ...
            _, prediction = _batch_metrics(output, label, device)
...
def _step_scheduler(scheduler, train_loss, epoch, result_file_path, observer):
...
def train(model, graph, ppi_list, loss_fn, loss_asl,
    ...
    for epoch in range(epochs):
        ...
        train_stats = _train_epoch(model, graph, loss_asl, optimizer, device, batch_size, epoch, got, observer)

        _save_checkpoint(model, epoch, os.path.join(save_path, 'gnn_model_train.ckpt'), observer)

        valid_stats = _validate_epoch(model, graph, loss_asl, device, batch_size, observer)

        observer.on_phase("Scheduler", "Updating learning rate from train epoch loss")
        _step_scheduler(scheduler, train_stats.loss, epoch, result_file_path, observer)

        if best_valid_f1 < valid_stats.f1:
            best_valid_f1 = valid_stats.f1
            best_valid_epoch = epoch
            _save_checkpoint(model, epoch, os.path.join(save_path, 'gnn_model_valid_best.ckpt'), observer)

        observer.on_epoch_end(epoch, train_stats, valid_stats, best_valid_f1, best_valid_epoch)

        _log_line(
            "epoch: {}, Training_avg: label_loss: {}, recall: {}, precision: {}, F1: {}, "
